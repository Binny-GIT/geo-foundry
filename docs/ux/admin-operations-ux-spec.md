...
> **状态：部分过时。** 本文件描述的是以 Payload 原生集合页为基础、按内容版本状态机组织的旧界面。现行产品方向、界面信息架构与导航结构见[产品说明](../product.md)，界面所有权见 [ADR 006](../adr/006-console-owns-ui-payload-headless.md)。
>
> 其中关于**权限范围、受限与空态区分、状态可解释性、不绕过服务端工作流、可访问性与响应式**的要求仍然有效，新界面继续遵守。涉及 Payload 原生列表、`/admin/collections/sites` 工作区和以状态机为主导航的部分已由新方案取代。
