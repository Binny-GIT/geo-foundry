...
每个 release 都是一组不可变对象：页面、sitemap、release manifest 与 routing 信息先被上传和验证，current pointer 最后以 ETag compare-and-swap 更新。对象 key、bytes、content type 和 SHA-256 必须与 manifest 一致。进入编译和 release envelope 的 edition 时间必须来自内容版本时间 `contentModifiedAt`，不能使用会被 audit、outbox 或其它控制面元数据写入刷新的 Payload `updatedAt`。
...
- CAS 成功但控制面 receipt 写入失败时，恢复流程必须 exact replay，不能 churn pointer ETag；重放必须复用相同的 `contentModifiedAt`，以保证 manifest 和 immutable objects byte-identical。
