...
const formatExtensions = new Set([".cjs", ".js", ".json", ".jsonc", ".mjs", ".ts", ".tsx"])
...
const changedFiles = () => {
  ...
  return [...files].filter((path) => formatExtensions.has(extname(path))).sort()
}
...
const main = async () => {
  ...
  const child = spawn("pnpm", ["exec", "biome", "format", "--error-on-warnings", ...files], {
    stdio: "inherit",
  })
...
main().catch((error) => {
  process.stderr.write(
    `${JSON.stringify({ code: error instanceof Error ? error.message : "CI_FORMAT_FAILED" })}\n`,
  )
  process.exitCode = 1
})
