...
await page.addInitScript(() => {
  window.__errors = []
  window.addEventListener("error", (event) => window.__errors.push(`error: ${String(event.message).slice(0, 300)}`))
...
const requested = await page.evaluate(() =>
    ...
    for (const element of document.body.querySelectorAll("*")) {
      for (const key of Object.keys(element)) {
        if (key.startsWith("__reactFiber$") || key.startsWith("__reactContainer$")) {
          withFiber.push(`${element.tagName.toLowerCase()}${element.id ? "#" + element.id : ""} ${key.startsWith("__reactContainer$") ? "CONTAINER" : "fiber"}`)
          break
        }
      }
  ...
  const sectionFiber = await page.evaluate(() => {
    ...
    for (let depth = 0; node !== null && depth < 14; depth += 1) {
      ...
      if (typeof type === "function" || typeof type === "object") {
        const named = type && (type.name || type.displayName || (type.$$typeof ? "lazy/forward" : "obj"))
        chain.push(`${depth}: ${String(named)} props=${JSON.stringify(Object.keys(node.memoizedProps ?? {})).slice(0, 140)}`)
      } else if (typeof type === "string") {
        chain.push(`${depth}: <${type}> children=${Array.isArray(node.memoizedProps?.children) ? node.memoizedProps.children.length : (node.memoizedProps?.children === null ? "null" : "single")}`)
      }
      node = node.return
    }
  ...
  firstBodyChildren: [...document.body.children].slice(0, 6).map((element) =>
    element.id ? `<${element.tagName.toLowerCase()} id=${element.id}>` : element.tagName.toLowerCase(),
  ),
...
console.log(JSON.stringify({ consoleMessages, failedRequests, initErrors, pageErrors, live, probe, reactAttachments: fibers, requestedChunks: requested, sectionFiber }, null, 2))
await browser.close()
