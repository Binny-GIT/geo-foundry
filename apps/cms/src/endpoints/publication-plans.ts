...
const errorOf = (error: unknown): Response => {
  if (error instanceof EditionWorkflowError)
    return response(error.code === "EDITION_WORKFLOW_PUBLISHER_REQUIRED" ? 403 : 409, {
      error: { code: error.code },
    })
  if (error instanceof PublicationPlansError) {
    const status =
      error.code === "PUBLICATION_PLAN_NOT_FOUND"
        ? 404
        : error.code.includes("PUBLISHER")
          ? 403
          : error.code.includes("CONFLICT") || error.code.includes("CANCELLABLE")
            ? 409
            : 400
...
export const createPublicationPlanEndpoint: Endpoint = {
  handler: async (req) => {
    if (resolveSessionClaims(req.user) === null)
      return response(401, { error: { code: "PUBLICATION_PLAN_UNAUTHENTICATED" } })
