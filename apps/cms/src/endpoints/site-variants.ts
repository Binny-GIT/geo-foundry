...
export const createSiteVariantEndpoint: Endpoint = {
  handler: async (req) => {
    if (resolveSessionClaims(req.user) === null) return response(401, { error: { code: "SITE_VARIANT_UNAUTHENTICATED" } })
    const editionId = editionIdOf(req)
    const parsed = bodySchema.safeParse(await req.json?.().catch(() => null))
    if (editionId === null || !parsed.success) return response(400, { error: { code: "SITE_VARIANT_BODY_INVALID" } })
    try {
      return response(201, await createSiteVariant(req.payload, { editionId, siteId: parsed.data.siteId, user: req.user }))
    } catch (error) {
      if (!(error instanceof SiteVariantError)) throw error
      const status = error.code === "SITE_VARIANT_NOT_FOUND" || error.code === "SITE_VARIANT_TENANT_MISMATCH" ? 404 : error.code === "SITE_VARIANT_EDITOR_REQUIRED" ? 403 : error.code === "SITE_VARIANT_ALREADY_EXISTS" || error.code === "SITE_VARIANT_TARGET_SAME_SITE" ? 409 : 400
