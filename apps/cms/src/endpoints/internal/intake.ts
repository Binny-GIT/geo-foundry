...
} from "../../server/repositories/intake-fetch"
...
const handleClaimIntakeFetch = withInternalGuards(
  ...
  async (req, ctx) => {
    await claimIntakeFetch(serverRuntime().db, intakeItemIdOf(req), req.user)
...
const handleGetIntakeFetchInput = withInternalGuards(
  ...
  async (req, ctx) =>
    internalJsonResponse(
      200,
      await readIntakeFetchInput(serverRuntime().db, intakeItemIdOf(req), req.user),
      ctx.requestId,
      null,
    ),
)
...
const handleCompleteIntakeFetch = withInternalGuards(
  ...
  async (req, ctx, body: IntakeFetchCompleteBody) => {
    ...
    const receipt = await completeIntakeFetch(serverRuntime().db, input, req.user)
