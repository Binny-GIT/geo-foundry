...
import {
  acceptPerformanceSuggestion,
  importPerformanceSnapshots,
  PerformanceSnapshotsError,
...
const csvImportOf = (text: string): unknown | null => {
  ...
  if (
    ...
    headers.some((header) => !CSV_HEADERS.includes(header as (typeof CSV_HEADERS)[number])) ||
    CSV_REQUIRED_HEADERS.some((header) => !headers.includes(header))
  )
    return null
...
const importBodyOf = async (
  req: CsvRequest & { json?: () => Promise<unknown> },
...
const errorOf = (error: unknown): Response => {
  ...
  const status = error.code.includes("UNAUTHENTICATED")
    ? 401
...
export const importPerformanceSnapshotsEndpoint: Endpoint = {
  handler: async (req) => {
    ...
    if (!parsed.success)
      return response(400, { error: { code: "PERFORMANCE_SNAPSHOT_BODY_INVALID" } })
    try {
      return response(
        201,
...
export const performanceSuggestionsEndpoint: Endpoint = {
  handler: async (req) => {
    try {
      return response(200, { suggestions: await performanceSuggestions(req.payload, req.user) })
    } catch (error) {
      return errorOf(error)
    }
  },
...
export const acceptPerformanceSuggestionEndpoint: Endpoint = {
  handler: async (req) => {
    ...
    if (!parsed.success)
      return response(400, { error: { code: "PERFORMANCE_SUGGESTION_BODY_INVALID" } })
    try {
      return response(
        201,
        await acceptPerformanceSuggestion(req.payload, { ...parsed.data, user: req.user }),
      )
    } catch (error) {
      return errorOf(error)
    }
