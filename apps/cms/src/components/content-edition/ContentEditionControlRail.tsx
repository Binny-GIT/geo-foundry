  ...
  return (
    <aside aria-label={lang === "zh" ? "任务控制和工作流" : "Editorial controls and workflow"} className="grid min-w-0 content-start gap-4">
      <section className="rounded-2xl border border-[var(--gf-border)] bg-[var(--gf-surface)] p-4 shadow-[var(--gf-shadow-surface)]">
        ...
        <div className="mt-4 grid gap-3"><label className="grid gap-1 text-xs font-bold text-[var(--theme-elevation-600)]">{t.owner}<select className="min-h-10 rounded-lg border border-[var(--theme-elevation-250)] bg-[var(--theme-elevation-50)] px-3 text-sm text-[var(--theme-text)]" disabled={readOnly} onChange={(event) => setOwner(event.target.value.length === 0 ? null : Number(event.target.value))} value={currentOwner}><option value="">—</option>{context.assignees.filter((assignee) => assignee.id !== null).map((assignee) => <option key={assignee.id} value={String(assignee.id)}>{assignee.email ?? assignee.id}</option>)}</select></label><label className="grid gap-1 text-xs font-bold text-[var(--theme-elevation-600)]">{t.priority}<select className="min-h-10 rounded-lg border border-[var(--theme-elevation-250)] bg-[var(--theme-elevation-50)] px-3 text-sm text-[var(--theme-text)]" disabled={readOnly} onChange={(event) => setPriority(event.target.value)} value={typeof priority === "string" ? priority : "normal"}><option value="low">{t.low}</option><option value="normal">{t.normal}</option><option value="high">{t.high}</option><option value="urgent">{t.urgent}</option></select></label><label className="grid gap-1 text-xs font-bold text-[var(--theme-elevation-600)]">{t.due}<input className="min-h-10 rounded-lg border border-[var(--theme-elevation-250)] bg-[var(--theme-elevation-50)] px-3 text-sm text-[var(--theme-text)]" disabled={readOnly} onChange={(event) => setDueAt(event.target.value.length === 0 ? null : new Date(event.target.value).toISOString())} type="datetime-local" value={localDateValue(dueAt)} /></label><label className="grid gap-1 text-xs font-bold text-[var(--theme-elevation-600)]">{t.editorial}<select className="min-h-10 rounded-lg border border-[var(--theme-elevation-250)] bg-[var(--theme-elevation-50)] px-3 text-sm text-[var(--theme-text)]" disabled={readOnly} onChange={(event) => setEditorialStatus(event.target.value)} value={typeof editorialStatus === "string" ? editorialStatus : "unassigned"}><option value="unassigned">{t.unassigned}</option><option value="assigned">{t.assigned}</option><option value="in-progress">{t.inProgress}</option><option value="blocked">{t.blocked}</option></select></label></div>
      </section>
      ...
      {canSchedule && <section className="rounded-2xl border border-[var(--gf-border)] bg-[var(--gf-surface)] p-4 shadow-[var(--gf-shadow-surface)]"><p className="m-0 text-xs font-extrabold uppercase tracking-[0.08em] text-[var(--gf-accent-700)]">{t.schedule}</p><label className="mt-3 grid gap-1 text-xs font-bold text-[var(--theme-elevation-600)]">{t.scheduleAt}<input className="min-h-10 rounded-lg border border-[var(--theme-elevation-250)] bg-[var(--theme-elevation-50)] px-3 text-sm text-[var(--theme-text)]" onChange={(event) => setScheduledFor(event.target.value)} placeholder="2026-12-01T15:00:00.000Z" value={scheduledFor} /></label><p className="m-0 mt-2 text-xs text-[var(--theme-elevation-600)]">{context.edition.siteTimezone}</p><button className="mt-3 min-h-10 rounded-lg bg-[var(--gf-accent-600)] px-3 text-sm font-bold text-white disabled:opacity-60" disabled={scheduledFor.length === 0 || scheduling} onClick={() => void schedule()} type="button">{t.schedule}</button></section>}
