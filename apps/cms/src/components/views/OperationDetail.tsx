...
const TEXT = {
  en: {
    ...
    failed:
      "This operation failed. Review the linked record before creating a new authorized attempt.",
    ...
    state: "State",
    technical:
      "Raw request payloads and provider diagnostics are intentionally kept in the protected ledger record.",
  },
  zh: {
    activity: "阶段时间线",
    attempt: "尝试次数",
...
export const OperationDetail = async ({
  i18n,
  initPageResult,
  params,
  payload,
  ...
  return (
    <WorkspaceShell i18n={i18n} kicker="Geo Foundry" title={t.operation}>
      <section
        className={`${cardClass} flex flex-col gap-5 p-5 sm:flex-row sm:items-start sm:justify-between`}
      >
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <h2 className="m-0 mt-1 font-mono text-xl font-bold tracking-tight text-[var(--theme-text)]">
              {display.operationId}
            </h2>
          </div>
        </div>
      ...
      <section className="grid gap-4 sm:grid-cols-4">
        {[
          ...
          <article className={`${cardClass} p-5`} key={label}>
            ...
            <strong className="mt-2 block break-words text-lg text-[var(--theme-text)]">
              {value}
            </strong>
          </article>
        ))}
        ...
        {display.timeline.length === 0 ? (
          ...
          <ol className="m-0 flex list-none flex-col p-0">
            {display.timeline.map((entry, index) => (
              <li
                className="flex gap-3 border-t border-[var(--theme-elevation-100)] py-3 first:border-t-0 first:pt-0"
              ...
              >
                ...
                <div className="min-w-0">
                  <strong className="block break-words text-sm text-[var(--theme-text)]">
                    {entry.stage ?? entry.action}
                  </strong>
                  <span className="mt-1 block text-xs text-[var(--theme-elevation-600)]">
                    {entry.at === null ? "—" : formatDate(entry.at, lang)}
                    {entry.actorRole === null ? "" : ` · ${roleLabel(entry.actorRole, lang)}`}
                  </span>
                </div>
              </li>
            ))}
      ...
      <section className={`${cardClass} flex flex-col gap-4 p-5`}>
        {display.state === "failed" && (
          <p className="m-0 text-sm font-semibold text-[var(--theme-error-700)]">{t.failed}</p>
        )}
        <p className="m-0 text-sm text-[var(--theme-elevation-600)]">{t.technical}</p>
        <WorkspaceAction href={`/admin/collections/operations/${idOf(operation) ?? id}`} primary>
          {t.openLedger}
        </WorkspaceAction>
      </section>
    </WorkspaceShell>
