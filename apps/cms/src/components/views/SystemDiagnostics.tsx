...
const TEXT = {
  en: {
    ...
    notReady: "Not ready",
    protected:
      "This page intentionally omits configuration values, credentials, request payloads, and provider prompts.",
...
export const SystemDiagnostics = async ({
  ...
  const pendingRollbacks = recordsOf(rollbackResult.docs).filter(
    (row) => row["consumedAt"] === null || row["consumedAt"] === undefined,
  )
  const dependencies = [
  ...
  return (
    <WorkspaceShell i18n={i18n} kicker="Geo Foundry" title={t.diagnostics}>
      <section className={`${cardClass} flex items-start gap-3 p-5`}>
        <IconBadge tone={readiness.status === "ready" ? "success" : "warning"}>
        ...
        <div>
          <h2 className="m-0 text-base font-bold text-[var(--theme-text)]">{t.readiness}</h2>
          <p className="m-0 mt-1 text-sm text-[var(--theme-elevation-600)]">{t.protected}</p>
        </div>
      </section>
      <section className="grid gap-4 sm:grid-cols-2">
        <article className={`${cardClass} flex flex-col gap-4 p-5`}>
          <h2 className="m-0 text-base font-bold text-[var(--theme-text)]">{t.dependencies}</h2>
          <ul className="m-0 flex list-none flex-col p-0">
            {dependencies.map(([name, dependencyStatus]) => (
              <li
                className="flex items-center justify-between gap-3 border-t border-[var(--theme-elevation-100)] py-3 first:border-t-0 first:pt-0"
                key={name}
              >
                <span className="text-sm font-semibold text-[var(--theme-text)]">{name}</span>
                <span
                  className={`rounded-full px-2 py-0.5 text-xs font-bold ${dependencyStatus === "ready" ? "bg-[var(--gf-tone-success-bg)] text-[var(--gf-tone-success-fg)]" : "bg-[var(--gf-tone-warning-bg)] text-[var(--gf-tone-warning-fg)]"}`}
                >
                  {status(dependencyStatus, lang)}
                </span>
              </li>
            ))}
          </ul>
        ...
        <article className={`${cardClass} flex flex-col gap-4 p-5`}>
          <div className="flex items-center gap-3">
            <IconBadge
              tone={
                failedOperations.length > 0 || pendingRollbacks.length > 0 ? "warning" : "success"
              }
            >
              <LayersIcon size={18} />
            </IconBadge>
            <h2 className="m-0 text-base font-bold text-[var(--theme-text)]">{t.risk}</h2>
          </div>
          <dl className="m-0 grid gap-3">
            <div className="flex items-center justify-between gap-3">
              <dt className="text-sm text-[var(--theme-elevation-600)]">{t.failedOperations}</dt>
              <dd className="m-0 text-xl font-bold tabular-nums text-[var(--theme-text)]">
                {failedOperations.length}
              </dd>
            </div>
            <div className="flex items-center justify-between gap-3">
              <dt className="text-sm text-[var(--theme-elevation-600)]">{t.rollback}</dt>
              <dd className="m-0 text-xl font-bold tabular-nums text-[var(--theme-text)]">
          ...
          <div className="flex flex-wrap gap-2">
            <WorkspaceAction href="/admin/collections/operations?where[state][equals]=failed">
              {t.failedOperations}
            </WorkspaceAction>
