      ...
      <header className="rounded-2xl border border-[var(--gf-border)] bg-[var(--gf-surface)] p-4 shadow-[var(--gf-shadow-surface)] sm:p-5">
        <div className="flex min-w-0 flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
          <div className="min-w-0">
            <p className="m-0 text-xs font-extrabold uppercase tracking-[0.08em] text-[var(--gf-accent-700)]">Geo Foundry · Content edition</p>
            <div className="mt-2 flex min-w-0 flex-wrap items-center gap-2">
              <h1 className="min-w-0 truncate text-xl font-bold tracking-tight text-[var(--theme-text)] sm:text-2xl">{typeof title === "string" && title.length > 0 ? title : lang === "zh" ? "未命名内容版本" : "Untitled content edition"}</h1>
              {isWorkflowStatus(workflowStatus) && <Badge tone={WORKFLOW_TONE[workflowStatus]}>{workflowStatusLabel(workflowStatus, i18n.language)}</Badge>}
            </div>
          ...
          {mode === "preview" ? (
            <div className="grid gap-4">
              <div className="rounded-2xl border border-[var(--gf-border)] bg-[var(--gf-surface)] p-5 shadow-[var(--gf-shadow-surface)] sm:p-7">
                <p className="m-0 text-xs font-extrabold uppercase tracking-[0.08em] text-[var(--gf-accent-700)]">{selectedVersion === null ? t.previewing : lang === "zh" ? "历史版本预览" : "Historical version preview"}</p>
                <h2 className="m-0 mt-2 text-3xl font-bold tracking-tight text-[var(--theme-text)]">{typeof source.title === "string" ? source.title : "—"}</h2>
                <p className="m-0 mt-3 max-w-3xl whitespace-pre-wrap text-base leading-7 text-[var(--theme-elevation-700)]">{typeof source.summary === "string" ? source.summary : "—"}</p>
              </div>
            ...
            <div className="grid gap-4">
              <div className="grid gap-4">
                ...
                <div className="rounded-2xl border border-[var(--gf-border)] bg-[var(--gf-surface)] p-5 shadow-[var(--gf-shadow-surface)] sm:p-7">
                  <p className="m-0 text-xs font-extrabold uppercase tracking-[0.08em] text-[var(--gf-accent-700)]">{t.editing}</p>
                  <div className="mt-5"><ContentEditionMetadataEditor readOnly={readOnly} /></div>
                </div>
              </div>
...
export const ContentEditionDocument = ({ formState }: DocumentViewClientProps) => {
  const { action, hasSavePermission, id, isEditing, isInitializing, isTrashed, setData } = useDocumentInfo()
  const { user } = useAuth()
  const { i18n } = useTranslation()
