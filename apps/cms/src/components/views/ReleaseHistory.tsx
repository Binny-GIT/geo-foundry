...
import { type RecordLike, recordsOf, stringOf } from "../dashboard/operations-model"
...
} from "../workspaces/lifecycle-workspace-model"
...
const TEXT = {
  en: {
    ...
    candidatesHint:
      "Only prior stable releases are listed. Restoring one always requires current-pointer preconditions and publisher approval.",
...
const short = (value: string | null) =>
  value === null ? "—" : value.length > 16 ? `${value.slice(0, 16)}…` : value

export const ReleaseHistory = async ({
  i18n,
  initPageResult,
  payload,
  user,
          ...
          {sites.map((site) => {
            ...
            return (
              <article className={`${cardClass} flex min-w-0 flex-col gap-5 p-5`} key={siteId}>
                <div className="flex w-full min-w-0 flex-col items-start gap-3 sm:flex-row sm:justify-between">
                  <div className="flex w-full min-w-0 items-start gap-3 sm:w-auto">
                    ...
                    <div className="min-w-0">
                      ...
                      <h2 className="m-0 mt-1 truncate text-lg font-bold text-[var(--theme-text)]">
                        {stringOf(site["name"], siteId)}
                      </h2>
                    </div>
                  </div>
                  <WorkspaceAction href={`/admin/collections/sites/${siteId}`}>
                    {t.openLedger}
                  </WorkspaceAction>
                </div>

                {current === undefined ? (
                  <p className="m-0 text-sm text-[var(--theme-elevation-600)]">{t.noCurrent}</p>
                ) : (
                  <div className="grid gap-3 rounded-xl border border-[var(--gf-accent-200)] bg-[var(--gf-tone-accent-bg)] p-4 sm:grid-cols-[minmax(0,1fr)_auto]">
                    <div className="min-w-0">
                      <span className="text-xs font-semibold text-[var(--gf-accent-700)]">
                        {t.current}
                      </span>
                      <strong className="mt-1 block truncate font-mono text-sm text-[var(--theme-text)]">
                        {current.releaseId}
                      </strong>
                      <span className="mt-1 block truncate text-xs text-[var(--theme-elevation-600)]">
                        {t.manifest} · {short(current.manifestSha256)}
                      </span>
                    </div>
                    <StatusBadge
                      label={releaseStateLabel(current.state, lang)}
                      state={current.state}
                    />
                  </div>
                )}
                ...
                <div className="grid min-w-0 grid-cols-1 gap-3 lg:grid-cols-2">
                  <section className="min-w-0">
                    <h3 className="m-0 text-sm font-bold text-[var(--theme-text)]">{t.release}</h3>
                    <ol className="m-0 mt-3 flex list-none flex-col p-0">
                      {history.map((release) => (
                        <li
                          className="flex min-w-0 items-center justify-between gap-3 border-t border-[var(--theme-elevation-100)] py-2.5 first:border-t-0 first:pt-0"
                        ...
                        >
                          <div className="min-w-0">
                            ...
                            <span className="block truncate text-xs text-[var(--theme-elevation-600)]">
                              {t.manifest} · {short(release.manifestSha256)}
                            </span>
                          </div>
                          <StatusBadge
                            label={releaseStateLabel(release.state, lang)}
                            state={release.state}
                          />
                        </li>
                      ))}
                  ...
                  <section className="min-w-0">
                    <div className="flex min-w-0 items-center gap-2">
                      ...
                      <h3 className="m-0 text-sm font-bold text-[var(--theme-text)]">
                        {t.candidates}
                      </h3>
                    </div>
                    ...
                    {candidates.length === 0 ? (
                      <p className="m-0 mt-3 text-sm text-[var(--theme-elevation-600)]">—</p>
                    ) : (
                      <ul className="m-0 mt-3 flex list-none flex-col p-0">
                        {candidates.map((candidate) => (
                          ...
                          >
                            <div className="min-w-0">
                              <strong className="block truncate font-mono text-sm text-[var(--theme-text)]">
                                {candidate.releaseId}
                              </strong>
                              <span className="block truncate text-xs text-[var(--theme-elevation-600)]">
                                {short(candidate.manifestSha256)}
                              </span>
                            </div>
                            <StatusBadge
                              label={releaseStateLabel(candidate.state, lang)}
                    ...
                    <div className="mt-4">
                      <WorkspaceAction href="/admin/collections/rollback-intents">
                        {t.rollback}
                      </WorkspaceAction>
                    </div>
                  </section>
