...
export const ConsoleCreateForm = ({ slug }: { readonly slug: ConsoleResourceSlug }) => {
  ...
  if (!CREATE_SUPPORTED.has(slug)) return null
  ...
  const submit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      window.location.assign(consoleRoute.collection(slug))
    } catch {
    ...
    <form className="grid gap-5" onSubmit={submit}>
        ...
        <>
          <label className="grid gap-2 text-sm font-medium text-[var(--console-ink)]">
            ...
            <small className="font-normal leading-5 text-[var(--console-ink-muted)]">
              填写有效 DNS 主机名，不含 https://、端口或路径。服务端会统一规范化并验证。
            </small>
          </label>
          <label className="grid gap-2 text-sm font-medium text-[var(--console-ink)]">
            所属站点
      ...
      <div className="flex flex-wrap justify-end gap-3 border-t border-[var(--console-border)] pt-5">
        <Button asChild size="lg" variant="secondary">
          <a href={consoleRoute.collection(slug)}>取消</a>
        </Button>
        <Button
