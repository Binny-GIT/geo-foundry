...
export const PerformanceSuggestions = ({ suggestions }: { readonly suggestions: readonly PerformanceSuggestion[] }) => {
  ...
  if (visible.length === 0) return <p className="m-0 px-5 py-6 text-sm text-[var(--console-ink-muted)]">No update suggestions right now.</p>
  ...
  return (
    <div className="grid">
      {error && <p className="m-0 border-b border-[var(--console-border)] bg-rose-50/60 px-5 py-2 text-xs text-rose-700 dark:bg-rose-400/10 dark:text-rose-300">Could not accept the suggestion. Try again.</p>}
      <ul className="m-0 list-none divide-y divide-[var(--console-border)] p-0">
        {visible.map((suggestion) => (
          <li key={suggestion.editionId} className="flex min-w-0 items-start justify-between gap-3 px-5 py-4">
