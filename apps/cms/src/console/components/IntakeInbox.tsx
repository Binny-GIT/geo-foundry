...
export const IntakeInbox = ({
  ...
  const setFilter = (key: "channel" | "status", value: string) => {
    ...
    startTransition(() =>
      router.replace(`/admin/inbox${params.size > 0 ? `?${params.toString()}` : ""}`),
    )
  }
    ...
    <div className="grid gap-6">
      ...
      {canManage && (
        ...
        >
          <div className="sm:col-span-full">
            ...
            <p className="m-0 pt-1 text-xs leading-5 text-[var(--console-ink-muted)]">
              URL 会先进入稿源箱并由后台安全抓取；确认后再采用为内容版本。
            </p>
          </div>
          ...
          <Button disabled={importing || sites.length === 0} size="lg" type="submit">
            <ImportIcon size={15} />
      ...
      <section className="gf-console-card grid min-h-[520px] overflow-hidden lg:grid-cols-[minmax(340px,0.95fr)_minmax(420px,1.35fr)]">
        <div className="border-b border-[var(--console-border)] lg:border-r lg:border-b-0">
          <div className="flex items-center justify-between border-b border-[var(--console-border)] px-5 py-4">
            <span className="text-sm font-semibold text-[var(--console-ink)]">
              {initialItems.length} visible items
            </span>
            {isPending && (
              <span className="text-xs text-[var(--console-ink-muted)]">Refreshing…</span>
            )}
          </div>
          {initialItems.length === 0 ? (
            <div className="grid min-h-64 place-items-center p-6 text-center">
              <div>
                <LayersIcon size={22} />
                <p className="m-0 pt-3 text-sm font-semibold text-[var(--console-ink)]">
                  No intake items found
                </p>
                <p className="m-0 pt-1 text-xs leading-5 text-[var(--console-ink-muted)]">
                  Try a different channel or status filter.
                </p>
              </div>
            </div>
          ) : (
            <ul className="m-0 max-h-[620px] list-none overflow-y-auto p-0">
              {initialItems.map((item, index) => {
                const id = String(item["id"] ?? index)
                const active = selected !== null && String(selected["id"]) === id
                return (
                  <li key={id}>
                    <Button
                      className={`block h-auto w-full rounded-none border-b border-[var(--console-border)] px-5 py-4 text-left font-normal transition-colors ${active ? "bg-indigo-50/80 dark:bg-indigo-400/10" : "hover:bg-[var(--console-surface-muted)]"}`}
                    ...
                    >
                      <div className="flex gap-3">
                        <span
                          className={`mt-1.5 size-2 shrink-0 rounded-full ${item["status"] === "failed" ? "bg-rose-500" : item["status"] === "ready" ? "bg-emerald-500" : "bg-indigo-500"}`}
                        />
                        <span className="min-w-0">
                          <strong className="block truncate text-sm text-[var(--console-ink)]">
        ...
        <div className="min-w-0 p-5 sm:p-6">
          {selected === null ? (
            <div className="grid min-h-64 place-items-center text-center">
              <div>
                <EyeIcon size={24} />
                <p className="m-0 pt-3 text-sm text-[var(--console-ink-muted)]">
                  Choose an intake item to preview it.
                </p>
              </div>
            </div>
          ) : (
            <div className="grid gap-5">
              <div className="flex items-start justify-between gap-4">
                ...
                <span className="rounded-full bg-[var(--console-surface-muted)] px-2.5 py-1 text-xs font-semibold text-[var(--console-ink-muted)]">
              ...
              <div>
                ...
                <p className="m-0 whitespace-pre-wrap pt-2 text-sm leading-6 text-[var(--console-ink-muted)]">
                  {text(selected["summary"], "No summary was provided.")}
                </p>
              </div>
              ...
              {notice && (
                <p
                  aria-live="polite"
                  className="m-0 rounded-xl bg-[var(--console-surface-muted)] p-3 text-sm text-[var(--console-ink-muted)]"
                >
                  {notice}
                </p>
              )}
                ...
                <div className="border-t border-[var(--console-border)] pt-5">
                  ...
                  <div className="flex flex-wrap gap-2 pt-3">
                    <Button
                      ...
                      type="button"
                    >
                      <CheckCircleIcon size={15} /> Adopt
                    </Button>
                    {(selected["status"] === "failed" ||
                      selected["failureCode"] === "INTAKE_QUEUE_UNAVAILABLE") && (
                      <Button
                    ...
                    <Button
                      disabled={isPending}
                      onClick={() => void operate("ignore")}
                      type="button"
                      variant="secondary"
                    >
                      <XIcon size={15} /> Ignore
                    </Button>
                  </div>
                  <div className="flex flex-col gap-2 pt-3 sm:flex-row">
                    <label className="sr-only" htmlFor="merge-target">
