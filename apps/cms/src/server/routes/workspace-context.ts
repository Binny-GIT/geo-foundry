...
export const handleWorkspaceContextGet = async (
  ...
  if (
    ...
    edition.tenantId !== (scope.kind === "global" ? null : scope.tenantId)
  ) {
    return json(404, { error: { code: "EDITION_WORKSPACE_NOT_FOUND" } })
  }
  const tenantId = edition.tenantId

  const [sourceRows, commentRows, assessmentRows, userRows] = await Promise.all([
    db
      .select({
  ...
  return json(200, {
    ...
    edition: {
      secondaryTopics: edition.secondaryTopics,
      sites: edition.sites,
