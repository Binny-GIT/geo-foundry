...
export const operations = geo.table(
  "operations",
  {
    ...
    attempt: numeric("attempt").default("1"),
    revision: numeric("revision").default("0"),
...
export const outboxEventType = pgEnum("enum_outbox_events_type", [
...
export const outboxAggregateType = pgEnum("enum_outbox_events_aggregate_type", ["edition", "site"])
export const outboxStatus = pgEnum("enum_outbox_events_status", ["pending", "dispatched"])

export const outboxEvents = geo.table(
  "outbox_events",
  {
    id: serial("id").primaryKey(),
    eventId: varchar("event_id").notNull(),
    type: outboxEventType("type").notNull(),
    aggregateType: outboxAggregateType("aggregate_type").default("edition").notNull(),
    aggregateId: numeric("aggregate_id").notNull(),
    tenantId: integer("tenant_id").notNull(),
    ...
    dispatchedAt: timestamp("dispatched_at", { withTimezone: true, precision: 3 }),
    updatedAt: timestamp("updated_at", { withTimezone: true, precision: 3 }).defaultNow().notNull(),
    createdAt: timestamp("created_at", { withTimezone: true, precision: 3 }).defaultNow().notNull(),
  },
  (table) => [
    ...
    index("outbox_events_operation_id_idx").on(table.operationId),
