...
describe("workbench server contract", () => {
  it("uses a bounded, scoped, URL-driven content edition query", async () => {
    ...
    expect(page).toContain("workDateRange")
    expect(page).toContain("workBoardEditions")
    expect(page).toContain("WORK_QUERY_LIMIT,")
    expect(page).toContain("siteScope: context.siteIds")
    ...
    expect(filters).toContain("toExclusive")
    expect(filters).toContain("shiftDays(today, 1)")
  ...
  it("keeps the workspace three-pane responsive with container queries under the console shell", async () => {
    ...
    // The editor lives in the console tree now: the 56px top bar comes from
    // ConsoleShell, no separate workspace top bar exists.
    expect(document).not.toContain("WorkspaceTopBar")
